...
XML = """
<mujoco model="single_arm_push_medium_bidirectional">
...
class MujocoSingleArmPushMediumBidirectionalEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.np_random.random() < 0.3:
            ...
            # left (조금 더 쉬운 범위)
            offset = self.np_random.uniform(
                low=np.array([-0.18, -0.10]),
                high=np.array([-0.10, 0.10]),
                size=(2,),
            )
    ...
    def step(self, action):
        ...
        ee_progress = float(self.prev_ee_obj_dist - ee_obj_dist)
        obj_progress = float(self.prev_obj_target_dist - obj_target_dist)

        reward = 0.2 * ee_progress
        reward += 2.5 * obj_progress

        # 잘못 밀면 추가 패널티
        if obj_progress < 0:
            reward += 1.5 * obj_progress

        if ee_obj_dist < self.contact_threshold_1:
            reward += 0.02
        if ee_obj_dist < self.contact_threshold_2:
            reward += 0.05
        if ee_obj_dist < self.contact_threshold_3:
            reward += 0.10

        if obj_target_dist < 0.16:
            reward += 0.03
        if obj_target_dist < 0.12:
            reward += 0.05
